...
class AnalyzeModelScene extends Scene {
    ...
    render() {
        ...
        let html = `
            <div class="summary">
                ...
                <p>${strings.analyzeModelSummary(Utils.Number.formatBytes(this.doc.model.size, 2), this.doc.model.columnsCount, this.doc.model.unreferencedCount)}</p>
            </div>
            <div class="fcols">
                <div class="col coll">
                    ...
                    <div class="toolbar">
                        ...
                        <div class="search">
                            <input type="search" placeholder="${strings.searchColumnPlaceholder}">
                        </div>

                       
                        <div class="filter-unreferenced toggle icon-filter-alerts" title="${strings.filterUnrefCtrlTitle}"></div>

                        <div class="group-by-table toggle icon-group" title="${strings.groupByTableCtrlTitle}"></div>

                        <hr class="show-if-group">

                        <div class="expand-all show-if-group ctrl icon-expand-all" title="${strings.expandAllCtrlTitle}"></div>
                    ...
                    <div class="warning-explanation">
                        ...
                        <p>${strings.columnWarningExplanation}</p>
                    </div>
    ...
    aggregateData(data) {
        ...
        aggregatedData.push({
            name: strings.aggregatedTableName,
            tableName: strings.aggregatedTableName,
            columnName: strings.otherColumnsRowName,
    ...
    updateData() {
        ...
        this.fullData = [];
        this.doc.model.columns.forEach(column => {
        ...
        if (!this.table) {
            ...
            const colConfig = {
                icon: { 
                    ...
                    formatter: (cell) => (cell.getData().isReferenced === false ? `<div class="icon icon-alert" title="${strings.columnWarningTooltip}"></div>` : ""), 
                    sorter: (a, b, aRow, bRow, column, dir, sorterParams) => (aRow.getData().isReferenced === false ? 1 : -1)
                },
                /*tickbox: {
                ...
                cardinality: { 
                    ...
                    title: strings.analyzeModelTableColCardinality,  
                    width: 120,
                    hozAlign:"right",
                    bottomCalc: "sum",
                    sorter: "number", 
                    headerTooltip: strings.analyzeModelTableColCardinalityTooltip,
                    formatter: (cell)=>Utils.Number.compress(cell.getValue()), 
                    bottomCalcFormatter: (cell)=>Utils.Number.compress(cell.getValue()),
                },
                entityName: { 
                    field: "columnName", 
                    title: strings.analyzeModelTableColEntity, 
                    cssClass: "column-name",
                },
                columnName: { 
                    field: "columnName", 
                    title: strings.analyzeModelTableColColumn, 
                    cssClass: "column-name",
                },
                tableName: { 
                    field: "tableName", 
                    title: strings.analyzeModelTableColTable,  
                    formatter: (cell) => (cell.getData()._aggregated ? "" : cell.getValue()),
                },
                size: { 
                    field: "size", 
                    title: strings.analyzeModelTableColSize, 
                    hozAlign:"right",
                    width: 100,
                    bottomCalc: "sum",
                    sorter: "number", 
                    formatter: (cell)=>{   
                        ...
                        let sizePerc = Math.round((value / (data._children ? this.topSize.tables : this.topSize.columns)) * 100);
                        return `${Utils.Number.formatBytes(value)}<div class="${data._children ? "size-indicator-alt" : "size-indicator"}" style="width:${sizePerc}%"></div>`;
                    }, 
                    bottomCalcFormatter: (cell)=>Utils.Number.formatBytes(cell.getValue()),
                },
                weight: { 
                    field: "weight", 
                    title: strings.analyzeModelTableColWeight, 
                    hozAlign: "right", 
                    width: 80,
                    bottomCalc: "sum",
                    sorter: "number", 
                    formatter: (cell)=>Utils.Number.formatPercentage(cell.getValue(), 0),
                    bottomCalcFormatter: (cell)=>Utils.Number.formatPercentage(cell.getValue(), 0)
                }
            };

            const columns = (this.groupByTable ?
            ...
            this.table.on("cellClick", (e, cell) => {
                try {
                    ...
                    let data = cell.getData();
                    if (data._aggregated && cell.getField() == "columnName") {
                        this.expandTableColumns();
                    }
                } catch(err) {}
            });
            this.table.on("rowClick", (e, row) => {
                let toggle = _(".tree-toggle", e.target);
                if (!toggle.empty) {
                    row.treeToggle();
                }
            });
                ...
                try {
                    ...
                    if  (this.chart) {
                        ...
                        for (let i = 0; i < chartData.length; i++) {
                            if (chartData[i].g == rowData.name || 
                                (chartData[i]._data.tableName == rowData.tableName && chartData[i]._data.columnName == rowData.columnName)) {
                                foundIndex = i;
                                break;
                            }
                        }
    ...
    chartItem(context) {
        ...
        if (isTable) {
            return {
                name: context.raw._data.tableName,
                tableName: context.raw._data.tableName,
                columnName: "",
                size: context.raw._data.size
            };
        } else {
            return context.raw._data.children[0];
        }
    }

    chartColors(context, type) {
    ...
    updateChart() {
       ...
       // container.style.height = `${document.body.clientHeight - container.offsetTop}px`;
        ...
        this.chart = new Chart(_("canvas", container).getContext("2d"), {
            type: "treemap",
            ...
            data: {
                datasets: [{
                    ...
                    borderWidth: 2,
                    spacing: 2,
                    color: context => this.chartColors(context, "color"),
                    backgroundColor: context => this.chartColors(context, "backColor"), 
                    hoverColor: context => this.chartColors(context, "hoverColor"),
                    ...
                    labels: {
                        ...
                        formatter(context) {
                            ...
                            if (context.raw) {
                                let item = context.raw._data.children[0];
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            title: context => {
                                let item = this.chartItem(context[context.length - 1]);
                            ...
                            footer: context => {
                                ...
                                if (item) {
                                    let lines = [];
                                    if (item.columnName) {
                                        lines.push(`${strings.analyzeModelTableColTable}: ${item.tableName}`);
                                    }
                                    lines.push(`${strings.analyzeModelTableColSize}: ${Utils.Number.formatBytes(item.size)}`);
                                    return lines;
                                }
                            },
                            label: context => false
    ...
    restoreTableRowsActiveStatus() {
        __(".tabulator-row", this.element).forEach(div => {
            div.classList.remove("active");
        });
    }

    updateToolbar() {
        __(".show-if-group", this.element).forEach(div => {
    ...
    update() {
        ...
        _(".summary p", this.element).innerHTML = strings.analyzeModelSummary(Utils.Number.formatBytes(this.doc.model.size, 2), this.doc.model.columnsCount, this.doc.model.unreferencedCount);
    }
    ...
    listen() {
        ...
        _(".filter-unreferenced", this.element).addEventListener("click", e => {
            ...
            this.showUnrefOnly = e.currentTarget.classList.contains("active");
        ...
        _(".group-by-table", this.element).addEventListener("click", e => {
            ...
            e.preventDefault();
            e.currentTarget.toggleClass("active");
            this.groupByTable = e.currentTarget.classList.contains("active");
