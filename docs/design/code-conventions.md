...
Follow the Microsoft [.NET naming guidelines](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/naming-guidelines) for names and [framework design guidelines](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/) for type and member contracts. Read the relevant section when introducing or changing that kind of API. Where existing code diverges from the applicable guideline, follow the guideline rather than copying the divergence. Apply explicit formatting and field-naming rules from [.editorconfig](../../.editorconfig).
...
Log without parsing the message. The level follows the outcome: `Warning` when the application continues unaffected,
